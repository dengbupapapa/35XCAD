...
export class Raycaster {
  ...
  static setContext(key, object) {
    Raycaster.context.set(key, object)
  }
  static getContext(key) {
    return Raycaster.context.get(key)
  }
  constructor(...args) {
  ...
  #pickPoint2Project(point, mouse, radius = config['point-size'] + 2) {
    let renderer = Raycaster.getContext('renderer')
  ...
  #intersectPoint(mouse, radius) {
    let select
    let pointsGeometryQuery = Raycaster.getContext('pointsGeometryQuery')
    let helpersGeometryMapper = Raycaster.getContext('helpersGeometryMapper')
    pointsGeometryQuery.all().forEach((point, index) => {
      let project = this.#pickPoint2Project(point, mouse, radius)
      if (project) {
        if (select) {
          if (
            helpersGeometryMapper.hasFormPoint(select.data) &&
            !helpersGeometryMapper.hasFormPoint(point)
          ) {
  ...
  #intersectLine(mouse, widthLine) {
    let select
    let linesGeometryQuery = Raycaster.getContext('linesGeometryQuery')
    let helpersGeometryMapper = Raycaster.getContext('helpersGeometryMapper')
    linesGeometryQuery.all().forEach((line, index) => {
      let project = this.#pickLine2Project(line, mouse, widthLine)
      if (project) {
        if (select) {
          if (
            helpersGeometryMapper.hasFormLine(select.data) &&
            !helpersGeometryMapper.hasFormLine(line)
          ) {
